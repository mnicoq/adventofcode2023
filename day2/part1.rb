...
# Go through all games, return sum of IDs of which are valid
def play(starting_cubes, input)
    sum = 0

    # Fetch the game
    id = 1
    input.each_line do |game|
        # Start fresh each game
        used_cubes = STARTING_CUBES.clone()

        # Convert the vile Game line into a usable hash
        # Ignore the first part to the left of :
        game = game.split(':')[1]
        parsed_plays = []

        # A game is a limited num of plays separated by ;
        raw_plays = game.strip.split(';')
        raw_plays.each do |raw_play|
            # puts raw_play
            # A play is a limited num of draws separated by ,
            raw_draws = raw_play.split(',')
            parsed_play = {}
            raw_draws.each do |raw_draw|
                # A draw is of the form quantity-color
                # where - is a ' ' character
                split_cube = raw_draw.strip.split(' ')
                value = split_cube[0].to_i
                key = split_cube[1].to_sym
                # Get the desired format
                parsed_play[key] = value
            end
            # Add to the list to be played in this game
            parsed_plays << parsed_play
        end

        # Start playing!
        parsed_plays.each do |play|
            # Start fresh each play
            # This copies values and not REFERENCE ?? otherwise a simple assignment keeps the same hash everytime
            used_cubes = STARTING_CUBES.clone()
            make_play(used_cubes, play)
            #puts "Game stance #{used_cubes}"
            # Don't keep playing if it's already invalid
            break unless is_valid?(used_cubes)
        end
  
        # After all possible plays have been made, see if we're valid
        #puts "Game #{id} ended: #{used_cubes}"
        if is_valid?(used_cubes)
            # If so, sum
            sum += id
        #else
...
# Make a play with some drawn cubes against a set.
# Modify the cube_set in place.
def make_play(cube_set, drawn_cubes)
    #puts "Make play #{drawn_cubes}"
    drawn_cubes.each do |k, v|
        cube_set[k] -= v
    end
end

# Check if a cube set is in a valid status.
def is_valid?(cube_set)
    return cube_set.all? { |k, v| v >= 0 }
end
...
puts play(STARTING_CUBES, input)
