# We don't care how we started, we want to max out
STARTING_CUBES = { "red": 0, "green": 0, "blue": 0 }
...
def play(starting_cubes, input)
    ...
    input.each_line do |game|
        ...
        # Start playing! make the MAX out of it
        parsed_plays.each do |play|
            make_play(used_cubes, play)
        end

        # Get the pow of the maxed set
        pow = get_power(used_cubes)
        sum += pow
    end
    sum
end

# "Make a play", which keeps the maximum amount of cubes used per colour
def make_play(cube_set, drawn_cubes)
    drawn_cubes.each do |k, v|
        cube_set[k] = v unless v <= cube_set[k]
    end
end

# Power of a set of cubes is equal to the numbers of red, green, and blue cubes multiplied together
def get_power(cube_set)
    # puts "Super set detected #{cube_set}"
    pow = 1
    cube_set.each do |k,v|
        pow = pow * v
    end
    pow
end
