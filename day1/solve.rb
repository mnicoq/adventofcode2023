...
def getCalibrations(input)
    total = 0
    input.each_line do |line|
        # Scan for values to calibrate
        # Took forever to remember about lookahead. Then we have to flatten cuz we have arrays of arrays
        possible_values = line.scan(/(?=(\d|one|two|three|four|five|six|seven|eight|nine))/).flatten
        # puts possible_values
        
        # Calibrate the first and last found
        first = getCalibrationValue(possible_values.first) * 10
        last = getCalibrationValue(possible_values.last)
        # puts "Line value = #{first + last}" 
        total += first + last 
    end
    total
end

def getCalibrationValue(value)
    # puts "Get value for: #{value}"
    text_numbers = { "one": 1, "two": 2, "three": 3, "four": 4, "five": 5, "six": 6, "seven": 7, "eight": 8, "nine": 9, "zero": 0 }
    
    return value.to_i if integer?(value)
    return text_numbers[value.to_sym]
end
